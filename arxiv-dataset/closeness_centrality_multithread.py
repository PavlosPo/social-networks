...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool() as pool:
        ...
        degree_centrality = pool.apply(calculate_closeness_centrality,(G,) )

    print('Outputs file named "closeness_centrality.txt"')
    # Save degree centrality results to a text file
    output_filename = 'closeness_centrality.txt'
    with open(output_filename, 'w') as f:
        for node, centrality in degree_centrality.items():
            f.write(f"Node: {node}, Centrality: {centrality}\n")

    print(f"Closeness centrality results saved to {output_filename}")
